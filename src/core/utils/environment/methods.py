...
from src.config.settings.static import CONFIGS_DIR
...
def collect_env_files_from_all_sources() -> Dict[str, str]:
    ...
    Источники:
    - Корневая директория проекта (ergo_ms/)
    - Папка configs и все её подпапки
    - Все вложенные папки в корневой директории
    ...
    try:
        ...
        project_root = os.path.dirname(os.path.abspath(CONFIGS_DIR))
        configs_dir = os.path.abspath(CONFIGS_DIR)
        
        all_env_files = []
        
        # 1. Собираем .env файлы из корневой директории проекта
        root_env_files = _find_env_files_in_directory(project_root, "корневая директория")
        all_env_files.extend(root_env_files)
        
        # 2. Собираем .env файлы из папки configs
        configs_env_files = _find_env_files_in_directory(configs_dir, "configs")
        all_env_files.extend(configs_env_files)
        
        if not all_env_files:
            logger.warning("Не найдено ни одного .env файла в проекте")
            return {}
        ...
        if not merged_vars:
            logger.warning("Не найдено ни одной переменной окружения")
            return {}
...
def get_env_sources() -> Dict[str, List[str]]:
    ...
    # Собираем .env файлы из всех источников
    root_env_files = _find_env_files_in_directory(project_root, "корневая директория")
    configs_env_files = _find_env_files_in_directory(configs_dir, "configs")
