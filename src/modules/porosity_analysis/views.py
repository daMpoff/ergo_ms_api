...
from src.modules.porosity_analysis.models import PorosityAnalysis, PorosityGroup, PorosityArchive, DownloadToken
from src.modules.porosity_analysis.serializers import (
    ...
    PorosityArchiveSerializer,
    CreatePorosityArchiveSerializer
)
...
class PorosityAnalysisViewSet(viewsets.ModelViewSet):
    ...
    def download_original(self, request, pk=None):
        ...
        try:
            # Используем FileResponse для потоковой передачи файла
            filename = os.path.basename(file_path)
            
            logger.info(f"Скачивание исходного изображения: анализ={analysis.id}, файл={filename}")
            
            # FileResponse автоматически делает потоковую передачу
            ...
            # Добавляем размер файла для отображения прогресса
            file_size = os.path.getsize(file_path)
            response['Content-Length'] = file_size
            
            return response
        except Exception as e:
            logger.error(f"Ошибка при чтении файла исходного изображения: {str(e)}")
            return Response({
    ...
    def generate_download_token(self, request, pk=None):
        """Генерирует временный токен для скачивания файлов анализа"""
        from datetime import timedelta
        
        analysis = self.get_object()
        file_type = request.data.get('file_type', 'report')  # report, original
        report_type = request.data.get('report_type', 'pdf')
        
        # Создаем токен со сроком действия 5 минут
        ...
        download_token = DownloadToken.objects.create(
            ...
            params={'report_type': report_type} if file_type == 'report' else {}
        )
        
        # Формируем URL для скачивания
        base_url = request.build_absolute_uri('/')[:-1]
        download_url = f"{base_url}/api/porosity_analysis/download/{download_token.token}/"
        
        return Response({
            'success': True,
            'token': str(download_token.token),
    ...
    def download_file(self, request, pk=None):
        ...
        try:
            with open(full_path, 'rb') as f:
                ...
                encoded_filename = urllib.parse.quote(os.path.basename(file_path).encode('utf-8'))
                response['Content-Disposition'] = f'attachment; filename="{os.path.basename(file_path)}"; filename*=UTF-8\'\'{encoded_filename}'
                return response
        except Exception as e:
            return Response({
                'error': f'Ошибка при чтении файла: {str(e)}'
    ...
    def _prepare_report_file_cached(self, analysis, report_type, file_cache):
        ...
        if len(file_cache) > 50:  # Максимум 50 файлов в кэше
            # Удаляем самые старые записи из кэша
            oldest_keys = list(file_cache.keys())[:10]  # Удаляем 10 самых старых
        ...
        try:
            ...
            if not report_file or not os.path.exists(report_file):
                return None, f"Анализ {analysis.id}: файл отчета типа {report_type} не найден"
    ...
    def _create_partial_archive(self, analyses_chunk, report_type, file_cache):
        ...
        try:
            ...
            with zipfile.ZipFile(archive_buffer, 'w', zipfile.ZIP_DEFLATED, compresslevel=1) as zip_file:  # Уменьшена степень сжатия для экономии памяти
                for analysis in analyses_chunk:
                    try:
                        ...
                        # Периодическая очистка памяти для больших чанков
            ...
            return result, successful_reports, failed_reports
    ...
    def _merge_archives_fast(self, archive_parts, info_content=None):
        ...
        try:
            ...
            with zipfile.ZipFile(final_buffer, 'w', zipfile.ZIP_DEFLATED, compresslevel=1) as final_zip:  # Уменьшена степень сжатия для экономии памяти
                ...
                for i, archive_data in enumerate(archive_parts):
                    ...
                    with zipfile.ZipFile(io.BytesIO(archive_data), 'r') as partial_zip:
                        for file_info in partial_zip.infolist():
                            ...
                            # Добавляем в финальный архив без папок (конфликты маловероятны из-за уникальных ID)
    ...
    def download_multiple_reports(self, request):
        ...
        try:
            ...
            # Убрано ограничение на количество анализов - система оптимизирована для обработки любых объемов
            ...
            # ОПТИМИЗИРОВАННАЯ ОБРАБОТКА ДЛЯ НЕОГРАНИЧЕННЫХ ОБЪЕМОВ
            from src.modules.porosity_analysis.config import PorosityAnalysisConfig
            
            # Определяем общее количество анализов
            total_analyses = len(analyses)
            
            # Инициализируем кэш файлов с ограничением
            file_cache = {}
            ...
            logger.info(f"Используются настройки архивирования: чанк={chunk_size}, потоки={merge_threads}")
            ...
            if total_analyses <= chunk_size:
                logger.info(f"Обработка {total_analyses} анализов в одном потоке (настройка chunk_size={chunk_size})")
                ...
                logger.info(f"Обработка {total_analyses} анализов в {merge_threads} потоках с чанками по {chunk_size} (настройки из конфига)")
                ...
                # Принудительная очистка памяти перед началом обработки
                gc.collect()
                ...
                # Очищаем частичные архивы из памяти
                archive_parts.clear()
                gc.collect()
            ...
            response['X-Archive-Chunk-Size'] = str(chunk_size)
            response['X-Archive-Merge-Threads'] = str(merge_threads)
            
            logger.info(f"Создан архив отчетов: {successful_reports} успешно, {len(failed_reports)} ошибок, метод: {'оптимизированный' if total_analyses > chunk_size else 'простой'}, настройки: chunk_size={chunk_size}, merge_threads={merge_threads}")
    ...
    def download_report(self, request, pk=None):
        ...
        if not report_files:
            return Response({
                'error': f'Отчет в формате {report_type} не найден. Убедитесь, что анализ завершен и отчеты сгенерированы.',
        ...
        try:
            ...
            logger.info(f"Скачивание отчета: анализ={analysis.id}, тип={report_type}, файл={download_filename}")
...
class PorosityArchiveViewSet(viewsets.ModelViewSet):
    """ViewSet для управления архивами отчетов пористости"""
    serializer_class = PorosityArchiveSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    filterset_fields = ['status', 'created_at', 'report_type']
    search_fields = ['name', 'description']
    ...
    def get_queryset(self):
        """Возвращаем queryset с сортировкой"""
        return PorosityArchive.objects.all().order_by('-created_at', 'id')
    
    def get_serializer_class(self):
        """Выбор сериализатора в зависимости от действия"""
        if self.action == 'create':
            return CreatePorosityArchiveSerializer
        return PorosityArchiveSerializer
    
    def create(self, request, *args, **kwargs):
        """Создание архива с асинхронной генерацией"""
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        # Если есть input_text, парсим его
        if input_text.strip():
            import re
            parsed_ids = []
            parts = re.split(r'[,;\s]+', input_text.strip())
            for part in parts:
                ...
                if '-' in part and not part.startswith('-'):
                    ...
                    parsed_ids.append(int(part))
            analysis_ids.extend(parsed_ids)
        ...
        # Получаем анализы
        analyses = PorosityAnalysis.objects.filter(id__in=analysis_ids, status='completed')
        existing_ids = list(analyses.values_list('id', flat=True))
        not_found = [i for i in analysis_ids if i not in existing_ids]
        
        if not analyses.exists():
            return Response({
                'success': False,
                'error': 'Не найдено завершенных анализов по указанным номерам'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # Создаем архив
        archive = PorosityArchive.objects.create(
        ...
        # Добавляем анализы в архив
        archive.analyses.set(analyses)
        
        # Запускаем асинхронную задачу создания архива
        from .tasks import create_archive_task
        create_archive_task.delay(archive.id)
        
        return Response({
            'success': True,
            'message': 'Архив создается',
            'data': PorosityArchiveSerializer(archive).data
        }, status=status.HTTP_201_CREATED)
    
    @action(detail=True, methods=['post'])
    def generate_download_token(self, request, pk=None):
        """Генерирует временный токен для скачивания архива"""
        from datetime import timedelta
        
        archive = self.get_object()
        
        if not archive.is_completed:
            return Response({
                'error': 'Архив еще не готов'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # Создаем токен со сроком действия 10 минут (больше времени для больших архивов)
    ...
    def download(self, request, pk=None):
        ...
        try:
            # Используем FileResponse для потоковой передачи файла
            # Используем UUID в имени файла для уникальности
            filename = f"{archive.uuid}.zip"
            
            logger.info(f"Скачиваем архив: UUID={archive.uuid}, файл={archive.file_path}, имя={filename}")
            
            # FileResponse автоматически делает потоковую передачу
            response = FileResponse(
                open(archive.file_path, 'rb'),
            ...
            # Добавляем размер файла для отображения прогресса
            file_size = os.path.getsize(archive.file_path)
            response['Content-Length'] = file_size
            
            return response
        except Exception as e:
            logger.error(f"Ошибка при чтении файла архива {archive.uuid}: {e}")
            return Response({
                'error': f'Ошибка при чтении файла: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def delete_archive(self, request, pk=None):
        ...
        try:
            # Удаляем файл архива, если он существует
            if os.path.exists(archive.file_path):
                os.remove(archive.file_path)
                logger.info(f"Удален файл архива: {archive.file_path}")
            ...
            logger.error(f"Ошибка при удалении архива {pk}: {e}")
            return Response({
                'success': False,
                'error': f'Ошибка при удалении архива: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def statistics(self, request):
        ...
        queryset = self.get_queryset()
        total = queryset.count()
        creating = queryset.filter(status='creating').count()
        completed = queryset.filter(status='completed').count()
        failed = queryset.filter(status='failed').count()
        
        # Общий размер всех архивов
        total_size = sum(archive.file_size or 0 for archive in queryset.filter(status='completed'))
...
class DownloadByTokenView(viewsets.ViewSet):
    """ViewSet для скачивания файлов по временному токену без авторизации"""
    permission_classes = []  # Публичный доступ по токену
    ...
    def download_by_token(self, request, token=None):
        ...
        try:
            # Получаем токен
            download_token = DownloadToken.objects.get(token=token_str)
            
            # Проверяем валидность
            if not download_token.is_valid:
                if download_token.is_expired:
                    return Response({
                        'error': 'Ссылка истекла. Пожалуйста, создайте новую ссылку для скачивания.'
                    }, status=status.HTTP_410_GONE)
                elif download_token.is_used:
                    return Response({
                        'error': 'Ссылка уже была использована.'
                    }, status=status.HTTP_410_GONE)
            
            # Обрабатываем скачивание в зависимости от типа
            if download_token.file_type == 'archive':
                return self._download_archive(download_token)
            elif download_token.file_type == 'report':
                return self._download_report(download_token)
            elif download_token.file_type == 'original':
                return self._download_original(download_token)
            else:
                return Response({
            ...
            logger.error(f"Ошибка при скачивании по токену {token_str}: {e}")
            return Response({
                'error': f'Ошибка при скачивании: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
    def _download_archive(self, download_token):
        """Скачивание архива"""
        try:
            archive = PorosityArchive.objects.get(id=download_token.object_id)
            ...
            logger.info(f"Скачивание архива по токену: UUID={archive.uuid}, файл={archive.file_path}")
    ...
    def _download_report(self, download_token):
        """Скачивание отчета"""
        try:
            analysis = PorosityAnalysis.objects.get(id=download_token.object_id)
            report_type = download_token.params.get('report_type', 'pdf')
            ...
            if not report_files:
                return Response({
                    'error': f'Отчет в формате {report_type} не найден'
                }, status=status.HTTP_404_NOT_FOUND)
            ...
            logger.info(f"Скачивание отчета по токену: анализ={analysis.id}, тип={report_type}")
    ...
    def _download_original(self, download_token):
        ...
        try:
            ...
            logger.info(f"Скачивание исходного изображения по токену: анализ={analysis.id}")
