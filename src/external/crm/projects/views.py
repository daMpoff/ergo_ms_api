...
class UserAllProjectsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Получаем все проекты пользователя через связь User_Project
            user_projects = User_Project.objects.filter(user_id=user_id).select_related('project')
            
            # Формируем ответ с обоими ID
            projects_data = [{
                'id': user_project.project.id,
                'name': user_project.project.name,
                'dateofcreation': user_project.project.dateofcreation,
                'creator_id': user_project.project.creator_id,  # ID создателя проекта из Project
                'user_id': user_project.user_id,               # ID пользователя из связи User_Project
...
class ProjectCreateView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Создаем проект через ORM
            project = Project.objects.create(
                name=project_name,
                creator=creator
            )
            
            # Формируем ответ с созданным проектом
            created_project = {
                'id': project.id,
                'name': project.name,
